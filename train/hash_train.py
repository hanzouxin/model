...
from utils.MSLOSS import MultiSimilarityLoss
from utils.BP import BPLoss
from utils.BPfuzhu import HardNegCLIPInfoNCE_Pro_NoLabel
...
class Trainer(TrainBase):
    ...
    def __init__(self,
                rank=get_args().device):
        args = get_args()
        
        super(Trainer, self).__init__(args, rank)
        
        self.ms_loss = MultiSimilarityLoss().to(self.args.device)
        self.bp = BPLoss(bit=self.args.output_dim).to(self.args.device)
        self.fuzhu=HardNegCLIPInfoNCE_Pro_NoLabel().to(self.args.device)
    ...
    def train_epoch(self, epoch):
        ...
        all_loss = 0
        times = 0
        for image, text, label, index,raw_text in self.train_loader:
            ...
            label = label.float()
            hash_img, hash_text,align_loss = self.model(image, text,label,raw_text)
            # loss1 = qmi_loss(images=hash_img, texts=hash_text, targets=label)
            # loss_img = self.ms_loss(hash_img, label)
            # loss_text = self.ms_loss(hash_text, label)
            # loss_i_t = self.ms_loss(hash_img, label, feat2=hash_text)
            ...
            loss1=intra_lossi+intra_losst+inter_loss
            loss2=self.fuzhu(hash_img, hash_text) 
            loss=loss1+0.8*loss2+align_loss
            # loss=loss1+align_loss
            all_loss += loss
    ...
    def get_code(self, data_loader, length: int):
        ...
        for image, text, label, index,raw_text in tqdm(data_loader):
            start_encoder_time = time.time()
            image = image.to(self.rank, non_blocking=True)
            text = text.to(self.rank, non_blocking=True)
            index = index.numpy()
            image_hash,text_hash = self.model.encoding(image,text,raw_text=raw_text, label=label)
